...
static	void	init_ray_direction(t_game *game, t_ray *ray)
{
	...
	{
		ray->step_x = -1;
		ray->side_dist_x = (game->player.x / game->tile_size - ray->map_x) * ray->delta_dist_x;
	}
	else
	{
		ray->step_x = 1;
		ray->side_dist_x = (ray->map_x + 1.0 - game->player.x / game->tile_size) * ray->delta_dist_x;
	}
	if (ray->dir_y < 0)
	{
		ray->step_y = -1;
		ray->side_dist_y = (game->player.y / game->tile_size - ray->map_y) * ray->delta_dist_y;
	}
	else
	{
		ray->step_y = 1;
		ray->side_dist_y = (ray->map_y + 1.0 - game->player.y / game->tile_size) * ray->delta_dist_y;
	}
}
...
void init_ray(t_game *game, t_ray *ray, float ray_angle)
{
	...
	// Position du joueur
	ray->map_x = (int)(game->player.x / game->tile_size);
	ray->map_y = (int)(game->player.y / game->tile_size);
